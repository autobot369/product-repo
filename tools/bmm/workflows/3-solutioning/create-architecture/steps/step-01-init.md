...
This will search Confluence space `{confluence_search_space}` for pages related to `{project_name}` (especially technical specs, API docs, backend references, and prior architecture decisions), fetch the most relevant ones, search Jira project `{jira_project}` for related epics and technical stories, and add all findings to frontmatter `inputDocuments`. Report findings alongside local document discovery.

#### C. Validate Required Inputs
