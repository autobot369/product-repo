...
This will search Confluence space `{confluence_search_space}` for pages related to `{project_name}`, fetch the most relevant ones, and add them to frontmatter `inputDocuments`. Report findings to the user alongside the local document discovery results.
