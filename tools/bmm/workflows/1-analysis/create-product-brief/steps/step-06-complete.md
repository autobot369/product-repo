...
### 3. Publish to Confluence and Jira
...
This will offer to:
- Publish the completed product brief to Confluence under Feature Grooming (space: `{confluence_publish_space}`, parent: `{confluence_feature_grooming_page_id}`)
- Apply labels: `{confluence_brief_labels}`
- Check for an existing page with the same title before creating a new one

Present options [P] Publish to Confluence / [S] Skip and wait for user selection before proceeding.
