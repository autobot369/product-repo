...
- Read fully and execute: `{project-root}/_bmad/bmm/tasks/remote-context-discovery.md` — search Confluence space `{confluence_search_space}` for existing API docs, backend specs, architecture pages, and technical references related to `{{research_topic}}`, and search Jira project `{jira_project}` for relevant technical epics before starting new research
