...
This will search Confluence space `{confluence_search_space}` for pages related to `{project_name}` (especially UX specs, design guidelines, and prior feature docs), fetch the most relevant ones, and add them to frontmatter `inputDocuments`. Report findings alongside local document discovery.
