...
# File References
# Route is determined by prd_intake_mode in config.yaml:
#   strategic-core → step-02-strategic-core.md  (single-pass intake, replaces 02+02b+02c+03)
#   step-by-step   → step-02-discovery.md        (original multi-turn facilitation)
nextStepFile_strategic_core: './step-02-strategic-core.md'
nextStepFile_step_by_step: './step-02-discovery.md'
...
#### B. Remote Context Discovery

**Load live Confluence and Jira context before proceeding:**

Read fully and execute: `{project-root}/_bmad/bmm/tasks/remote-context-discovery.md`

This will search Confluence space `{confluence_search_space}` for pages related to `{project_name}`, fetch the most relevant ones, search Jira project `{jira_project}` for related in-flight issues, and add all findings to frontmatter `inputDocuments`. Report findings to the user alongside the local document discovery results.
...
- IF C: Update output file frontmatter, adding this step name to the end of the list of stepsCompleted, then:
    - Read `prd_intake_mode` from config.yaml
    - IF `prd_intake_mode` = `strategic-core` → read fully and follow: {nextStepFile_strategic_core}
    - IF `prd_intake_mode` = `step-by-step` (or unset) → read fully and follow: {nextStepFile_step_by_step}
...
ONLY WHEN [C continue option] is selected and [frontmatter properly updated with this step added to stepsCompleted and documentCounts], will you then route to the correct next step:
- `prd_intake_mode: strategic-core` → `{nextStepFile_strategic_core}` (single-pass Strategic Core intake)
- `prd_intake_mode: step-by-step` → `{nextStepFile_step_by_step}` (multi-turn discovery)
