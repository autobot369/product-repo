...
This will offer to:
- Publish the completed PRD to Confluence under Feature Grooming (space: `{confluence_publish_space}`, parent: `{confluence_feature_grooming_page_id}`)
- Apply labels: `{confluence_prd_labels}`
- Create Jira epics and stories in project `{jira_project}` with label `{jira_story_label}`
- Check for existing pages/issues before creating duplicates

Present options [P] Confluence / [J] Jira Issues / [B] Both / [S] Skip and wait for user selection before proceeding.
